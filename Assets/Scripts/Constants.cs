public static class Constants
{
    public const float TookSpeedRotation = 630f;
    public const float PikeSpeedRotation = 600f;
    public const float EntranceSpeedRotation = 30f;

    public const float ClockwiseDirection = -1;
    public const float CounterClockwiseDirection = 1;

    public const byte DefoltJumpForce = 7;
    public const byte JumpForceFromArmStand = 5;

    public const float DeltaXInJump = 2f;
    public const float ColliderYSizeInStandWirhArmsExtended = 5.5f;
    public const float DefoltColliderYSize = 4.6f;
    public const float DeltaHeightInOtherStands = -0.3f;
    public const float NoDeltaHeight = 0f;
    public const float DeltaXInChangeStand = 0.005f;

    public const float RiseTimeMark = 0.8f;
    public const float HeightRiseMark = 5;
    public const int AngleForTenPoints = 3;
    public const float RefereeingError = 0.5f;

    public const float DiverX = 5.43f;
    public const float DiverY = 22.29f;
    public const float MarkY = -10.4f;
    public const float MarkX1 = 4.5f;

    public const float LevelWater = 0f;
    public const float CameraMinLevel = LevelWater + 1.5f;

    public const float MinMark = 0f;
    public const float MaxMark = 10f;

    public const float TimeSplash = 0.2f;
    public const float MinSplashScaleX = 0.4f;
    public const float MaxSplashScaleX = 3.5f;
    public const float MinSplashScaleY = 0.3f;
    public const float MaxSplashScaleY = 2f;
    public const float MinSplashLifeTime = 0.0025f;
    public const float MaxSplashLifeTime = 0.5f;
}